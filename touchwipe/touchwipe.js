...
(function ($) {
    $.fn.touchWipe = function (option) {
        var defaults = {
            ...
            delWidth = $(opts.itemDelete).width(),
            initX,
            initY,
            moveX,
            moveY,
        ...
        $(opts.itemDelete).on('click',function(){
            opts.delCallback && opts.delCallback.call(this);
            // $(this).parent().remove();
        })
        ...
        function setScrollX(event) {
            ...
            if (Math.abs(X) > Math.abs(Y)) {
                flagX = 1;//是否是左右滑动 0为初始，1为左右，2为上下，在move中设置，在end中归零
            } else {
                flagX = 2;
            }
            return flagX;
